...
# inform_1, inform_4, inform_5 항목별로 count해서 막대 그래프로 시각화
st.subheader("항목별 데이터 분포")
fig, ax = plt.subplots(3, 1, figsize=(10, 15))

# inform_1 count plot
eqp_vend_nm_counts = df['inform_1'].value_counts()
...
# inform_4 count plot
sido_nm_counts = df['inform_4'].value_counts()
...
# inform_10을 5000 단위로 그룹화하여 Rate로 끝나는 항목들은 표준편차 방법, 나머지는 IQR 방법으로 이상치 분석 수행 후 정리
df['inform_10_bin'] = pd.cut(df['inform_10'], bins=np.arange(0, df['inform_10'].max() + 5000, 5000), right=False, include_lowest=True)
...
non_rate_columns = [col for col in df.columns if col not in rate_columns]

# 각 그룹에 대해 이상치 분석 수행
for name, group in df.groupby('inform_10_bin', observed=True):
    group_summary = {'inform_10_bin': name}
...
classification_outliers_summary_df = classification_outliers_summary_df.loc[:, ~classification_outliers_summary_df.columns.duplicated()]
